...
The engine ships seven models — **ITU-R P.1812**, **ITM/Longley-Rice**, **ITU-R P.528** (air-to-ground, used for Op E with drone C2), **ITU-R P.530**, **ITU-R P.526** as plugins; **free-space (Friis)** and **two-ray ground reflection** as core-bundled non-removable models. The auto-select strategy filters by frequency range, scores by `(operation, link_type, geometry) → scenario` suitability via the frozen mapping table in §4.4, and down-weights any model whose required data tier exceeds what the AOI provides. Callers may pin a specific model. Each registered model declares an SPDX `license` and a `provenance` string, both copied into `Run.models_used[]` so every Run is license-auditable from its record alone. Bundled link-type plugins: `lora`, `lte`, `drone_c2`, `rtk`, `vhf_telemetry`. Why a registry rather than hardcoded dispatch — see [ADR-0003](docs/adr/0003-propagation-model-registry.md).
